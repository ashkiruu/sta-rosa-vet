...
class CertificateService
{
     ...
     * FIXED: Now also updates the appointment status to 'Completed' for report sync
     */
    public static function approveCertificate($certificateId, $approvedBy = 'Admin')
    {
        ...
        // BUGFIX: Sync appointment status to 'Completed' so reports count it correctly
     ...
     * This ensures that reports correctly count completed services
     */
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .no-print button { background: #dc2626; color: white; border: none; padding: 12px 30px; font-size: 16px; cursor: pointer; border-radius: 5px; margin: 0 10px; }
        .no-print button:hover { background: #b91c1c; }
        .certificate-container { max-width: 800px; margin: 0 auto; background: white; box-shadow: 0 0 20px rgba(0,0,0,0.1); }
        .certificate { padding: 40px; border: 3px solid #1e40af; margin: 10px; position: relative; }
        .certificate::before { content: ''; position: absolute; top: 5px; left: 5px; right: 5px; bottom: 5px; border: 1px solid #1e40af; pointer-events: none; }
        .header { text-align: center; margin-bottom: 30px; border-bottom: 2px solid #1e40af; padding-bottom: 20px; }
        .header-top { display: flex; justify-content: center; align-items: center; gap: 20px; margin-bottom: 10px; }
        .logo { width: 80px; height: 80px; background: #1e40af; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 40px; }
        .header h1 { color: #1e40af; font-size: 28px; text-transform: uppercase; letter-spacing: 2px; }
        .header h2 { color: #dc2626; font-size: 22px; margin-top: 10px; text-transform: uppercase; letter-spacing: 3px; }
        ...
        .certificate-number strong { color: #1e40af; }
        .section { margin-bottom: 25px; }
        .section-title { background: #1e40af; color: white; padding: 8px 15px; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 15px; }
        .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px 30px; }
        .info-row { display: flex; border-bottom: 1px dotted #ccc; padding: 5px 0; }
        .info-label { font-weight: bold; color: #333; min-width: 150px; font-size: 13px; }
        .info-value { color: #1e40af; font-size: 13px; }
        .info-row.full-width { grid-column: span 2; }
        .certification-text { text-align: center; padding: 20px; background: #f8fafc; border: 1px solid #e2e8f0; margin: 20px 0; font-style: italic; line-height: 1.8; }
        .signature-section { display: flex; justify-content: space-between; margin-top: 40px; padding-top: 20px; }
        .signature-box { text-align: center; width: 45%; }
        .signature-line { border-top: 2px solid #333; padding-top: 10px; margin-top: 50px; }
        .signature-name { font-weight: bold; font-size: 14px; text-transform: uppercase; }
        .signature-title { font-size: 12px; color: #666; }
        .footer { text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #ccc; font-size: 11px; color: #666; }
        .watermark { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-30deg); font-size: 100px; color: rgba(30, 64, 175, 0.05); font-weight: bold; pointer-events: none; white-space: nowrap; }
        .stamp { position: absolute; bottom: 100px; right: 60px; width: 100px; height: 100px; border: 3px solid #dc2626; border-radius: 50%; display: flex; align-items: center; justify-content: center; transform: rotate(-15deg); opacity: 0.8; }
        .stamp-inner { text-align: center; color: #dc2626; font-weight: bold; font-size: 11px; text-transform: uppercase; }
    </style>
</head>
<body>
    <div class="no-print">
    ...
    <div class="certificate-container">
        <div class="certificate">
            ...
            <div class="certificate-number">Certificate No: <strong>{$certificateNumber}</strong></div>
            <div class="section">
                <div class="section-title">Pet Information</div>
                <div class="info-grid">
                    <div class="info-row"><span class="info-label">Pet's Name:</span><span class="info-value">{$petName}</span></div>
                    <div class="info-row"><span class="info-label">Type of Animal:</span><span class="info-value">{$animalType}</span></div>
                    <div class="info-row"><span class="info-label">Gender/Sex:</span><span class="info-value">{$petGender}</span></div>
                    <div class="info-row"><span class="info-label">Age:</span><span class="info-value">{$petAge}</span></div>
                    <div class="info-row"><span class="info-label">Breed:</span><span class="info-value">{$petBreed}</span></div>
                    <div class="info-row"><span class="info-label">Color:</span><span class="info-value">{$petColor}</span></div>
                    <div class="info-row full-width"><span class="info-label">Date of Birth:</span><span class="info-value">{$petDob}</span></div>
                </div>
            </div>
            <div class="section">
                <div class="section-title">Owner Information</div>
                <div class="info-grid">
                    <div class="info-row full-width"><span class="info-label">Owner's Name:</span><span class="info-value">{$ownerName}</span></div>
                    <div class="info-row full-width"><span class="info-label">Complete Address:</span><span class="info-value">{$ownerAddress}</span></div>
                    <div class="info-row"><span class="info-label">Civil Status:</span><span class="info-value">{$civilStatus}</span></div>
                    <div class="info-row"><span class="info-label">Years of Residency:</span><span class="info-value">{$yearsOfResidency}</span></div>
                    <div class="info-row"><span class="info-label">Contact Number:</span><span class="info-value">{$ownerPhone}</span></div>
                    <div class="info-row"><span class="info-label">Birthdate:</span><span class="info-value">{$ownerBirthdate}</span></div>
                </div>
            </div>
            <div class="section">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-row"><span class="info-label">Vaccine Used:</span><span class="info-value">{$vaccineUsed}</span></div>
                    <div class="info-row"><span class="info-label">Date of Vaccination:</span><span class="info-value">{$vaccinationDate}</span></div>
                    <div class="info-row"><span class="info-label">Lot/Batch Number:</span><span class="info-value">{$lotNumber}</span></div>
                    <div class="info-row full-width"><span class="info-label">Next Vaccination:</span><span class="info-value">{$nextVaccinationDate}</span></div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="info-grid">
                    <div class="info-row full-width"><span class="info-label">Veterinarian:</span><span class="info-value">{$veterinarianName}</span></div>
                    <div class="info-row"><span class="info-label">License No.:</span><span class="info-value">{$licenseNumber}</span></div>
                    <div class="info-row"><span class="info-label">PTR No.:</span><span class="info-value">{$ptrNumber}</span></div>
                </div>
            </div>
            <div class="signature-section">
                <div class="signature-box"><div class="signature-line"><div class="signature-name">{$veterinarianName}</div><div class="signature-title">Licensed Veterinarian</div></div></div>
                <div class="signature-box"><div class="signature-line"><div class="signature-name">City Veterinarian</div><div class="signature-title">Head of Office</div></div></div>
            </div>
            <div class="stamp"><div class="stamp-inner">VERIFIED<br>✓<br>CVO</div></div>
            <div class="footer">
