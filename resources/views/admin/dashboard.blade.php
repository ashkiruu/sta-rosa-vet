...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    {{-- Pending Residents - visible to Staff & Admin --}}
    ...
    {{-- Today's Appointments - visible to Doctor & Admin --}}
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
    <div class="bg-white rounded-[2rem] shadow-sm border border-purple-100 p-6 flex items-center border-l-8 border-l-purple-600">
        ...
        <div>
            <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Staff & Doctors</p>
            <h3 class="text-2xl font-black text-gray-900 leading-none">{{ $stats['total_staff'] ?? 0 }}</h3>
        </div>
    </div>
...
<div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {{-- Doctor: Certificates --}}
        @if($role === 'doctor')
        <a href="{{ route('admin.certificates.index') }}" class="flex flex-col items-center justify-center p-6 bg-emerald-50 rounded-3xl border border-transparent hover:border-emerald-200 hover:shadow-md transition group text-center">
            <div class="w-12 h-12 bg-white rounded-2xl shadow-sm flex items-center justify-center text-emerald-600 mb-3 group-hover:bg-emerald-600 group-hover:text-white transition">
                <i class="fas fa-certificate"></i>
            </div>
            <span class="text-[10px] font-black text-gray-900 uppercase tracking-widest">Certificates</span>
        </a>
        @endif

        {{-- Staff + Doctor: Reports --}}
        ...
        <a href="{{ route('admin.admins.create') }}" class="flex flex-col items-center justify-center p-6 bg-gray-900 rounded-3xl border border-transparent hover:bg-black hover:shadow-lg transition group text-center">
            ...
            <span class="text-[10px] font-black text-white uppercase tracking-widest">Assign Role</span>
        </a>

        <a href="{{ route('admin.logs') }}" class="flex flex-col items-center justify-center p-6 bg-purple-50 rounded-3xl border border-transparent hover:border-purple-200 hover:shadow-md transition group text-center">
            <div class="w-12 h-12 bg-white rounded-2xl shadow-sm flex items-center justify-center text-purple-600 mb-3 group-hover:bg-purple-600 group-hover:text-white transition">
                <i class="fas fa-history"></i>
            </div>
            <span class="text-[10px] font-black text-gray-900 uppercase tracking-widest">Activity Logs</span>
        </a>
        @endif
    </div>
</div>
